...
public class PlayActivity extends ActionBarActivity implements OnClickListener,
	...
	private class RandomRecommendTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... voids) {
			for (int i = 0; i < mRandomJsonArray.length(); i++) {
				...
				try {
					JSONObject animationObject = mRandomJsonArray
							.getJSONObject(i);
					Animation animation = Animation.build(animationObject);
					...
					View line = mRecommendView.findViewById(R.id.divide_line);
	...
	private JsonHttpResponseHandler mRandomHandler = new JsonHttpResponseHandler() {
		public void onSuccess(int statusCode, org.json.JSONObject response) {
			if (statusCode == 200) {
				try {
					JSONArray animations = response.getJSONObject("data")
							.getJSONObject("list").getJSONArray("anime");
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		mAnimation.checkIsFavorite(new Animation.UpdateFinishCallback() {
					@Override
					public void onUpdateFinished(Animation.Method method,
							Message msg) {
						if (msg.arg1 == 1) {
							runOnUiThread(new Runnable() {
								...
								public void run() {
									mFavMenuItem
											.setIcon(R.drawable.ab_fav_active);
								}
							});
						}
					}
	...
	private void startPlayAnimation(final int start, Animation animation) {
		...
		final String playUrl = mSharedPreferences.getBoolean("use_hd", true) ? mAnimation.HDVideoUrl
				: mAnimation.CommonVideoUrl;
		DownloadRecord record = DownloadRecord.getFromAnimation(mAnimation,
				true);
		if (record != null) { 
			...
			if (f.isFile() && f.exists()) {
				...
				Toast.makeText(mContext, R.string.offline_file_missing,
		...
		switch (item.getItemId()) {
			...
			case R.id.action_fav: 
				if (mAnimation.isFavorite()) {
					mAnimation
							.removeFromFavorite(new Animation.UpdateFinishCallback() {
								@Override
								public void onUpdateFinished(
										Animation.Method method, Message msg) {
									Toast.makeText(mContext,
											R.string.fav_del_success,
											Toast.LENGTH_SHORT).show();
								}
							});
					...
					mAnimation
							.addToFavorite(new Animation.UpdateFinishCallback() {
								@Override
								public void onUpdateFinished(
										Animation.Method method, Message msg) {
									Toast.makeText(mContext,
											R.string.fav_success,
											Toast.LENGTH_SHORT).show();
								}
							});
	...
	public void onBitmapFailed() {
		// 代码冗余，已经在menu创建的时候初始化
		// if (mShareActionProvider != null) {
		// mShareActionProvider.setShareIntent(getDefaultIntent());
		// }
		mLoadingGif.setPaused(true); 
	...
	private class CommentsTask extends AsyncTask<Void, LinearLayout, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			AVQuery<AVObject> query = new AVQuery<AVObject>("Comments");
	...
	private void initContent() {
		...
		new Thread() { 
			...
			public void run() {
				super.run();
				DownloadRecord record = DownloadRecord.getFromAnimation(
						mAnimation, true);
				if (record != null) {
	...
	private void pausePlay() {
		...
		if (mVV == null || mVV.isPlaying() == false) {
			return;
		}
	...
	private void setMaxSize() {
		...
		if (Build.MODEL.equals("M040")) {
			if (!mSharedPreferences.getBoolean("Meizu", false)) {
				...
				superToast.setIcon(SuperToast.Icon.Dark.INFO,
	...
	public void onCompletion() {
		...
		/**
		 * --------modified by mooncafes-----------------
		 * onCompletion方法，在onPause方法后被调用，如果，在视频正在播放或者暂停播放视频的条件下,
		 * 此时，app进入后台，然后再进入前台时，会被调用一次，导致mLastPos = 0 被调用一次，
		 * 这样，在onResume的时候，mLastPos永远为0，
		 */
		if (STATE.PAUSE == mState) { 
			return;
		}
